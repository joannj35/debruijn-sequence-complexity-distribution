...
class SequenceGenerator {
    ...
    void sequencesFromIndices(const vector<int>& , int index, const string& current);
public:
    int getNumOfSeq() const;

    explicit SequenceGenerator(int complexity, const vector<char> &alphabet = {'0', '1'}) :
                                                                    seq_len(pow(2,ceil(log2(complexity)))),
