...
void ComplexityToDebruijn::compute() {
    ...
    this->up_to_1000 =  new char*[1000 * num_sub_seq];
...
#pragma omp target data map(to: flattened_data[0:num_sub_seq*sub_size], num_sub_seq, sub_size) map(tofrom: subseq_to_db_flatten[0:num_sub_seq]) map(tofrom: up_to_1000_flatten[0:num_sub_seq*1000])
    {
        // Parallelize the loop
        #pragma omp target teams distribute parallel for schedule(dynamic) shared(flattened_data,sub_size,num_sub_seq, sub_seq_flatten, n, up_to_1000_flatten,subseq_to_db_flatten) private(i) default(none) num_teams(1) thread_limit(64)
        for (i = 0; i < num_sub_seq; i++) {
            ...
            seq[sub_size] = '\0';
            char** db_seq = up_to_1000_flatten + (1000*i);
    ...
    for (int c = 0; c < num_sub_seq; c++){
        ...
        char* str = up_to_1000_flatten[c * 1000];
        int s = 0;
//        while (str != nullptr && s < 1000) {
//            fileout << str << endl;
//            str++;
//            s++;
//        }
